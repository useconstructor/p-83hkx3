...
"En app/page.tsx, en la lista de plantas, agrega un botón 'Editar' para cada planta. Al hacer click, almacena el id de esa planta en un estado editingId y muestra un pequeño formulario inline con los valores actuales de nombre, precio y descripción prellenados. Al hacer click en 'Guardar', hace fetch PUT a /api/plants/${plant.id} con los nuevos valores y actualiza el array local de plantas. Al hacer click en 'Cancelar', limpia editingId para ocultar el formulario."
...
- Run: npm run build
- If it passes: commit all changed files with message "change: En app/page.tsx, en la lista de plantas, agrega un botón 'Ed" and push
